...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    <%
      ...
      Dim uid, uemail, uphone, ulocation, category, department, description, contact,  entered_by
      Dim kb
    ...
    ' Write problem into database
      ...
      strProblemQry = "INSERT INTO problems (id, uid, uemail, uphone, ulocation, entered_by, " & _
        "category, department, description, contact, priority, status, start_date, rep, time_spent, kb) " & _
        "VALUES (" & id & ",'" & uid & "','" & uemail & "','" & uphone & "','" & _
        ulocation & "'," & entered_by & "," & category & "," & department & ",'" & _
        description & "','" & contact & "'," & priority & "," & status & "," & start_date & "," & rep & "," & time_spent & _
        "," & kb & ")"
    ...
    <div align="center">
      <table class="Wide">
        <tr class="Normal">
          <td><div align="right"><a href="print.asp?id=<% = id %>" target="printwindow"><%=lang(cnnDB, "PrinterFriendly")%></a></div></td>   		</tr>
        <tr class="Head1">
          <td>
            <%=lang(cnnDB, "Problem")%>&nbsp;<% = id %>&nbsp;<%=lang(cnnDB, "Submitted")%>          </td>
        </tr>
        <tr class="Body1">
          <td>
            <table class="Wide">
              ...
              <tr>
                <td>
                  <b><%=lang(cnnDB, "Category")%>:</b>                </td>
                <td>
                  <% = catRes("cname") %>                </td>
              </tr>
              <tr>
                <td>
                  <b><%=lang(cnnDB, "AssignedTo")%>:</b>                </td>
                <td>
                  <a href="mailto:<% = Usr(cnnDB, rep, "email1") %>?Subject=<%=lang(cnnDB, "HELPDESK")%>: <%=lang(cnnDB, "Problem")%> <% = id %>"><% = Usr(cnnDB, rep, "fname") %></a>                </td>
              </tr> 
               <tr>
                <td>
                  <b><%=lang(cnnDB, "Contact")%>:</b>                </td>
                <td>
                  <% = contact %>                </td>
              </tr>            
            </table>          </td>
        ...
        <tr class="Head2">
          <td>
            <%=lang(cnnDB, "Description")%>:          </td>
        </tr>
        <tr class="Body1">
          <td>
            <center><form><textarea name="display_desc" rows="10" cols="80"><% = description %></textarea></form></center>          </td>
        </tr>
      </table>
    </div>
