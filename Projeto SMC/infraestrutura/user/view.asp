...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    <%
      ...
      listStr = "SELECT TOP 50 p.id, p.category, p.start_date, r.fname, r.email1 As remail, s.sname " & _
      ...
      Select Case intSort
        Case 1  ' id
          ...
          End If		
        Case 2  ' category
          listStr = listStr & " ORDER BY p.category"
          If intOrder = 0 Then
            listStr = listStr & " ASC"
            intTitleOrder = 1
          Else
    ...
    <div align="center">
      <table class="Wide">
        <tr class="Head1">
          <td colspan="5">
            <%=lang(cnnDB, "ProblemListingfor")%>&nbsp;<% = uid %>          </td>
        </tr>
        <tr class="Head2" align="center">
          ...
          <td nowrap=><a href="view.asp?start=<% = start %>&num=<% = numToDisplay %>&sort=2&order=<% = intTitleOrder %>" class="HeadLink"><%=lang(cnnDB, "category")%></a></td>
  ...
  Dim Categorias, retornoCategoria 
  Set Categorias = SQLQuery(cnnDB, "SELECT cname FROM categories WHERE category_id=" & rstProbList("category"))
  If Categorias.EOF Then
    retornoCategoria = rstProbList("category")
  Else
    retornoCategoria = Categorias("cname")
  End If
  Categorias.Close
        ...
        %>
              <tr class="Body1">
                ...
                <td nowrap><div align="center"><a href="details.asp?id=<% = rstProbList("id") %>"><% = retornoCategoria %> </a></div></td>
                <td nowrap><div align="center"><a href="mailto:<% = rstProbList("remail") %>?Subject=HELPDESK: Problem <% = rstProbList("id") %>">
                  <% = rstProbList("fname") %>
                </a></div></td>
                <td nowrap><div align="center"><% = rstProbList("start_date") %></div></td>
        ...
        %>
          ...
          <tr class="Body1">
            <td colspan="5">
              <div align="center">
                <b><%=lang(cnnDB, "Noresultsfound")%>.</b>              </div>            </td>
          </tr>
        <% End If %>
      </table>
    </div>
