...
Sub DisplayError(eType, component)
  ...
  If Not Application("Debug") Then
    Response.Clear
    Response.Write("<html><head><title>ERRO</title></head><body>")
  End If
  Response.Write "<p><center><table width=""200""><tr><td bgcolor=""red"" align=""center"">" & _
	  "<b>ERRO</b></tr></td><tr><td bgcolor=""#eeeeee"" align=""center"">"
	...
	Select Case Err.Number
		...
		Case -2147217904 ' sql: invalid column
			strInformation = "The SQL query references a invalid column or object.  Read the description above" & _
				" and check the code."
		Case Else
			strInformation = "Não há mais informações disponíveis."
	End Select

	strInformation = strInformation & vbNewLine & _
		"<p>Retorne a " & _
		"<a href=""http://sistemas.smc:89"">página inicial</a>."
...
Sub DisplayHeader(cnnDB, sid)
	...
	If Usr(cnnDB, sid, "IsRep2") = 1 Then
	...
	If Usr(cnnDB, sid, "IsRep2") = 1 or Session("IsAdmin") Then

		If Session("IsRep2") AND Session("IsAdmin") Then
			Response.Write(lang(cnnDB, "SupportRepAdminloggedin") & ".<br />")
		Elseif Usr(cnnDB, sid, "IsRep2") = 1 Then
...
Sub DisplayFooter(cnnDB, sid)
  Dim userChkRes
    ...
    If Usr(cnnDB, sid, "IsRep2") > 0 Then
	...
	Response.Write"<p><hr width=""500"">" & vbNewLine & _
	  ...
	  "<a href=""http://sistemas.smc"">" & lang(cnnDB, "LiberumHelpDesk") & "</a>, " & lang(cnnDB, "Copyright") & ". " & lang(cnnDB, "Pleaseviewthe") & " <a href=""" & Cfg(cnnDB, "BaseURL") & "/ajuda.asp"">" & lang(cnnDB, "license") & "</a>." & _
	  "</font></div></p>"
	userChkRes.Close
End Sub
...
'
' CheckRep:
' Checks to see if IsRep2 is TRUE, If not, returns
...
Sub CheckKB(cnnDB, sid)
  ...
  Select Case Cfg(cnnDB, "EnableKB")
    ...
    Case 1
      Dim rstRep
      Set rstRep = SQLQuery(cnnDB, "SELECT IsRep2 FROM tblUsers WHERE sid=" & sid)
      If Cint(rstRep("IsRep2")) = 1 Then
        blnOK = True
      Else
        blnOK = False
      End If
