...
<%
  ...
  Dim uid, uemail, uphone, ulocation, category, department, contact, description, kb
  ...
  ' Check for required fields (uemail, category, department, description)
  ...
  strProbQuery = "SELECT uid, uemail, uphone, ulocation, time_spent, department, " & _
		"category, status, priority, entered_by, rep, kb, start_date, close_date, contact, description " & _
...
<tr>
    ...
    <em>*</em> = <%=lang(cnnDB, "Required")%> |
	  ...
	  <a href="print2.asp?id=<% = id %>" target="printwindow">O.S. Teste</a>
	</td>
</tr>
<tr class="Head1">
	<td colspan="2">
...
<tr class="Body1">
    ...
    <div align="center">
      ...
      <tr>
        ...
        <td>
          <a href="mailto:<% = uemail %>?Subject=HELPDESK: Problem <% = id %>"><% = uid %> - <% = Usr(cnnDb, entered_by, "fname") %></a></td>
      </tr>
      <tr>
        <td>
          <b><%=lang(cnnDB, "EMail")%>:</b> <em>*</em></td>
        <td>
          <input type="text" name="uemail" size="40" value="<% = uemail %>" <% = strTextDisable %>></td>
      </tr>
      <tr>
        <td>
      ...
      <tr>
        ...
        <td>
          <input type="text" name="uphone" size="10"value="<% = uphone %>"  <% = strTextDisable %>>        </td>
      </tr>
      <tr>
      	<td><b><%=lang(cnnDB, "Contact")%>:</b></td>
        <td><% = contact %></td>
      </tr>
    </table>
    <hr/>
    <table class="Wide">
      <tr>
        <td align="center" colspan="4">
          <b><%=lang(cnnDB, "Classification")%></b></td>
      </tr>
      <tr>
        <td width="102">
          <b><%=lang(cnnDB, "Category")%>:</b> <em>*</em></td>
        <td colspan="3"><select name="category" <% = strListDisable %>>
          <%
            Dim rstCat
      ...
      <tr>
        <td>
          <b><%=lang(cnnDB, "Status")%>:</b> <em>*</em></td>
        <td width="180">
        ...
        <td width="122">
          <b><%=lang(cnnDB, "Priority")%>:</b> <em>*</em></td>
        <td width="196">
      ...
      <tr>
        <td>
          <b><%=lang(cnnDB, "AssignTo")%>:</b> <em>*</em></td>
        <td>
          <SELECT NAME="rep" <% = strListDisable %>>
          <%
            Dim rstRes
            Set rstRes = SQLQuery(cnnDB, "SELECT * From tblUsers WHERE IsRep2=1 AND RepAccess<>2 ORDER BY uid ASC")
        ...
        <td>
          <b><%=lang(cnnDB, "TimeSpent")%>:</b>        </td>
        <td>
          <input type="text" size="4" name="time_spent" value="<% = time_spent %>"  <% = strTextDisable %>> 
          (<%=lang(cnnDB, "minutes")%>)        </td>
      </tr>
      <tr>
        <td>
          <b><%=lang(cnnDB, "StartDate")%>:</b>        </td>
        <td>
          <% = DisplayDate(start_date, lhdDateTime) %>        </td>
        <td>
          <b><%=lang(cnnDB, "CloseDate")%>:</b>        </td>
        <td>
          <% = DisplayDate(close_date, lhdDateTime) %>        </td>
      </tr>
    </table>
    <% If status = Cfg(cnnDB, "CloseStatus") Then %>
      <div align="center">
...
<tr class="Body1">
  <td colspan="2">
    ...
    <% If Cfg(cnnDB, "EnableKB") <> 0 Then
         If kb = 0 Then %>
          <br/><input type="checkbox" name="kb" <% = strListDisable %>>&nbsp;<%=lang(cnnDB, "EnterinKnowledgeBase")%>
        <% Else %>
          <br/><input type="checkbox" name="kb" checked <% = strListDisable %>>&nbsp;<%=lang(cnnDB, "EnterinKnowledgeBase")%>
        <% End If %>
    <% End If %>
  </td>
</tr>
