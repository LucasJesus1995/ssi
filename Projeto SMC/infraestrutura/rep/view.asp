...
<%
  ...
  Set repRes = SQLQuery(cnnDB, "SELECT uid FROM tblUsers WHERE sid=" & rep_id )
	...
	listStr = "SELECT TOP 100 p.id, p.category, p.start_date, p.uid, p.uemail, r.uid AS ruid, pri.pname, s.sname " & _
...
<div align="center">
  ...
  <tr align="center" Class="Head2">
    ...
    <td><a href="view.asp?rep_id=<% = rep_id %>&start=<% = start %>&num=<% = numToDisplay %>&sort=2&order=<% = intTitleOrder %>" class="HeadLink"><%=lang(cnnDB, "Category")%></a></td>
 ...
 <% End If %>
  ...
  Set Categorias = SQLQuery(cnnDB, "SELECT cname FROM categories WHERE category_id=" & listRes("category"))
  If Categorias.EOF Then
    retornoCategoria = listRes("category")
  Else
  ...
  %>
    <tr align="center" valign="center" class="Body1">
      ...
      <td><A HREF="details.asp?id=<% = listRes("id") %>"><% = retornoCategoria %></A></td>
  ...
  <%
    ...
    End If
  %>
      <td nowrap><% = listRes("start_date") %></td>
